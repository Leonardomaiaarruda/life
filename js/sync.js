...
window.Sync = (() => {
  ...
  function render() {
    ...
    host.querySelector('span').textContent=queue.length ? (running ? 'Salvando alterações…' : queue[0].blocked ? 'Alteração pendente: o servidor recusou o envio. Confira os dados ou entre novamente.' : `${queue.length} alteração(ões) pendente(s). Aguardando envio ao servidor.`) : 'Alterações salvas no servidor.';
  ...
  async function save(action,data) {
    ...
    render();await flush(false);
    if(owner()!==id)return {ok:false,stale:true};
    return read(id).some(item=>item.id===entry.id) ? {ok:true,pending:true} : {ok:true};
  }
