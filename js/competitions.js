window.Competitions=(()=>{
  ...
  const rules=c=>`${metrics[c.metric]} · Limite de ${c.daily_cap} por pessoa/dia · ${c.photo_required?'Foto obrigatória':'Foto opcional'} · ${c.allow_manual===false?'Apenas atividades importadas':'Check-ins manuais permitidos'}`;
  ...
  async function submit(form){
    ...
    try{
      ...
      const item={...fields,id:draftId,weights,members,photo_required:form.elements.photo_required.checked,allow_manual:form.elements.allow_manual.checked,teams:document.getElementById('competitionTeamNames').value.split('\n').map(v=>v.trim()).filter(Boolean)};
  ...
  document.addEventListener('click',async e=>{
    ...
    if(action==='comments'){await Community.comments(button.dataset.post);return;}
    if(action==='chat'){const c=items.find(c=>c.id===selected);await Community.chat('competition:'+selected,c.title);return;}
    ...
    try{
      if(action==='react'){await request('setSocialReaction',{post_id:button.dataset.post,kind:'clap'});toast('Aplauso registrado.');return;}
      if(action==='photo'){
